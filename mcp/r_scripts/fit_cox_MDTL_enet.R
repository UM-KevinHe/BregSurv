...
if (length(args) < 2) {
  stop("Usage: Rscript fit_cox_MDTL_enet.R <input.json> <output.json>")
}
input_path  <- args[1]
output_path <- args[2]
...
result <- tryCatch({
  ...
  # Only override the R-side default ifelse(n<p, 0.05, 1e-3) if user supplied one.
  if (!is.null(lambda.min.ratio)) enet_args$lambda.min.ratio <- lambda.min.ratio

  fit <- do.call(cox_MDTL_enet, enet_args)
