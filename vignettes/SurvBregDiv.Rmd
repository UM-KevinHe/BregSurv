---
title: "SurvBregDiv: Bregman Divergence Data Integration for Time-to-Event Modelling"
output:
...
vignette: >
  %\VignetteIndexEntry{SurvBregDiv: Bregman Divergence Data Integration for Time-to-Event Modelling}
...
```{r, include=FALSE}
knitr::opts_chunk$set(fig.alt = "Plot generated in SurvBregDiv vignette")
library(dplyr)
```
...
The following flowchart illustrates the overall workflow of the `SurvBregDiv` package. Users first choose an integration strategy based on the type of external information available, then select a study design (full-cohort Cox model or nested case–control). The package supports both low-dimensional estimation and high-dimensional penalized regression, with a suite of cross-cutting features applicable across methods.
```{r, echo=FALSE, eval=TRUE, out.width="100%"}
knitr::include_graphics("../man/figures/SurvBregDiv_flowchart.svg")
```
...
In the NCC setting, event times are represented through matched case--control sets, and estimation proceeds via conditional logistic regression. `SurvBregDiv` enables external information borrowing within this framework through Bregman divergence, allowing improved efficiency while respecting the matched-set structure. Standard matched case--control or matched cohort studies can be viewed as special cases of the NCC design. For methodology details, please refer to Page: **Appendix: NCC KL Divergence**.
