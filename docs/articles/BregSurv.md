...
- **Coefficient + curvature summaries**: when
  ...
  additional positive semidefinite matrix $`\mathbf{A}`$ (e.g., the
  information / precision matrix of the external estimator) is provided,
...
3.  [Summary-level External Coefficients / risk scores + Curvature (via
    ...
    along with a positive semidefinite matrix $`\mathbf{A}`$ (e.g., the
    information / precision matrix), the user can perform data
    integration via the quadratic Mahalanobis distance.
...
function
[`coxkl()`](https://um-kevinhe.github.io/BregSurv/reference/coxkl.md) to
...
(e.g. [`cv.coxkl_enet()`](https://um-kevinhe.github.io/BregSurv/reference/cv.coxkl_enet.md))
...
function (e.g., an information / precision matrix, or curvature
...
$`\widetilde{\boldsymbol{\beta}}`$ is supplied to the function through
the argument `beta =`, and the external curvature (precision) matrix
$`\mathbf{A}`$ through the argument `Q =`. Below, we illustrate the
usage when no external $`\mathbf{A}`$ matrix is provided by setting
`Q = NULL`.
...
difference is that users should use the functions
...
- **Mahalanobis distance with external curvature** (analog of
  ...
  $`\widetilde{\boldsymbol{\beta}}`$ plus its precision (information)
  matrix `Q`.
...
ncckl.fit_breslow <- ncckl(y = y.cc, z = z.cc, stratum = set.cc, 
                           etas = eta_list, beta = beta_ext.cc,
                           method = "breslow")
```
